...
class MyCanvasPolyline : public QCanvasLine
{
public:
    MyCanvasPolyline(QCanvas *canvas) : QCanvasLine(canvas) {QCanvasLine::setPen(NoPen); kol=0; keyPoint=0;};
    QPointArray &getPointArray(){return poly;};
...
{
public:
    DrawCanvas( QCanvas *canvas, QWidget *parent,
         long k,
                //char *infX, char *infY,
    ...
    bool resetData(bool flag=false); // сброс установленных данных для графиков
...
protected:
    ...
    QCanvasLine      *lineMetka[KolMetka];   // метки под графиком
