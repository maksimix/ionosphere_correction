...
DrawBaseCanvas::DrawBaseCanvas(QCanvas *viewing, QWidget *parent,
                               //char *infX, char *infY,
                               ...
                               fildView(QRect(0,0,1,1))
{
  ...
  canvasText=new QCanvasText(canvas);
  ...
  fildDiagramm=new QCanvasRectangle(canvas);
  fildDiagramm->setPen(NoPen);
  fildDiagramm->setZ(zFild);
  fildDiagramm->show();

  colorLineFild=colorFon;
  QBrush brush( colorLineFild, SolidPattern );
  QPen pen;
  pen.setColor(colorLineFild);
  pen.setCapStyle(FlatCap);
  pen.setWidth(1);
  
  lineFild=new QCanvasRectangle(canvas);
  lineFild->setPen(pen);
  ...
  {
      lineMarkFild[i]=new QCanvasRectangle(canvas);
  ...
  lineSelectFild=new QCanvasRectangle(canvas);
...
void DrawBaseCanvas::contentsMouseDoubleClickEvent ( QMouseEvent * e )
{
    ...
    if (e->button()==LeftButton)
    {
        ...
        if (flagIn)
        {
            ...
            kolMarkFild=k;
            double x1, x2;
            x1=xmin+(xmax-xmin)*(pointBegin.x()-size.width())/(nx-otst);
            x2=xmin+(xmax-xmin)*(pointEnd.x()-size.width())/(nx-otst);
            for(i=0; i<kolMarkFild; i++)
            {
                x1=xmin+(xmax-xmin)*(lineMarkFild[i]->x()-size.width())/(nx-otst);
                x2=xmin+(xmax-xmin)*((lineMarkFild[i]->x()+lineMarkFild[i]->width())-size.width())/(nx-otst);
...
void DrawBaseCanvas::contentsMousePressEvent ( QMouseEvent * e )
{
    ...
    if (flagKeyShift || e->button()==MidButton) // движется мышка и нажата клавиша Shift (показываем маркер)
    {
    ...
    if (e->button()==LeftButton)
    {
        ...
        if (flagKeyCtrl)
        {
            ...
            int  i,k;
            /*
            double x1=xmin+(xmax-xmin)*(pointBegin.x()-size.width())/nx;
            cout<<endl<<"posX="<<x1<<flush;
            ...
            emit signalDelSector(x1);
            return;
            */
            for(i=0; i<kolMarkFild; i++)
            {
                if ( pointBegin.x() >= lineMarkFild[i]->x() && pointBegin.x() <= lineMarkFild[i]->x()+lineMarkFild[i]->width() )
                {
                    lineMarkFild[i]->setSize(0,0);
                    flagIn=true;
                }
...
void DrawBaseCanvas::contentsMouseReleaseEvent ( QMouseEvent * e )
{
    ...
    if (flagMouseMidPress)
    {
        ...
        canvasText->setX(.0);
        canvasText->setY(.0);
        canvasMarker->setCenterMarker(ix,iy);
        canvasMarker->hide();
    }
    ...
    if (flagMouseLeftPress)
    {
        if (!flagKeyShift)
        {
            ...
            for(int i=0; i<maxKolMarkFild; i++)
            {
                ...
                {
                    if ( pointBegin.x() <= lineMarkFild[i]->x() && pointEnd.x() >= lineMarkFild[i]->x()+lineMarkFild[i]->width() )
                        lineMarkFild[i]->setSize(0,0);
                    else
                    {
                        if ( pointBegin.x() < lineMarkFild[i]->x() &&
                             pointEnd.x() > lineMarkFild[i]->x() &&
                             pointEnd.x() < lineMarkFild[i]->x()+lineMarkFild[i]->width() )
                        {
                            x2=xMarkEnd[i];
                            pointEnd.setX(lineMarkFild[i]->x()+lineMarkFild[i]->width());
                            lineMarkFild[kolMarkFild]->setSize(pointEnd.x()-pointBegin.x(),ny);
                            lineMarkFild[i]->setSize(0,0);
                        }
                        else
                        {
                            if ( pointEnd.x() > lineMarkFild[i]->x()+lineMarkFild[i]->width() &&
                                 pointBegin.x() > lineMarkFild[i]->x() &&
                                 pointBegin.x() < lineMarkFild[i]->x()+lineMarkFild[i]->width() )
                            {
                                x1=xMarkBegin[i];
                                pointBegin.setX(lineMarkFild[i]->x());
                                lineMarkFild[kolMarkFild]->setX(lineMarkFild[i]->x());
                                lineMarkFild[kolMarkFild]->setY(lineMarkFild[i]->y());
                                lineMarkFild[kolMarkFild]->setSize(pointEnd.x()-pointBegin.x(),ny);
                                lineMarkFild[i]->setSize(0,0);
                            }
                            else
            ...
            if (flagIn)
            {
                ...
                for(i=0; i<kolMarkFild; i++)
                {
                    emit signalChangeMarkFild(i, xMarkBegin[i], xMarkEnd[i]);
                }
            }
        }
    }
...
{
    if (flagKeyShift || flagMouseMidPress) // движется мышка и нажата клавиша Shift (показываем маркер)
    {
        pointMetka=e->pos();
    ...
    if (!flagKeyShift && flagMouseLeftPress)
    {
        if ( pointBegin.x() < pointEnd.x() )
        {
...
{
    //cout<<endl<<"n="<<n<<" kolMarkFild="<<kolMarkFild<<flush;
    if (n>=maxKolMarkFild)
        return;
...
{
    cout<<endl<<"keyRelease="<<e->key()<<flush;
    switch(e->key())
    {
    case Key_Control:
        flagKeyCtrl=false;
...
{
    cout<<endl<<"keyPress="<<e->key()<<flush;
    switch(e->key())
    {
    case Key_Control:
        flagKeyCtrl=true;
...
void DrawBaseCanvas::slotSetMarkerHide()
{
    canvasMarker->setCenterMarker(ix,iy);
    canvasMarker->hide();
    update();
...
bool DrawBaseCanvas::eventFilter ( QObject * obj, QEvent * e )
{
    QCanvasView::eventFilter ( obj, e );
    //cout<<endl<<"event="<<e->type()<<flush;
    switch(e->type())
    {
    case QEvent::Wheel:
        {
            //cout<<endl<<"event="<<e->type()<<"  delta="<<((QWheelEvent*)e)->delta()<<flush;
    ...
    case QEvent::KeyRelease:
        {
            //cout<<endl<<"F keyRelease="<<((QKeyEvent*)e)->key()<<flush;
    ...
    case QEvent::KeyPress:
        {
            //cout<<endl<<"F keyPress="<<((QKeyEvent*)e)->key()<<flush;
            switch( ((QKeyEvent*)e)->key())
            {
            case Key_Control:
                flagKeyCtrl=true;
...
MyRectCanvasSetka::MyRectCanvasSetka(QCanvas *canvas, QRect &r,
                                     double *Xmin, double *Xmax, double *Ymin, double *Ymax,
                                     long *iX, long *iY, long *nX, long *nY,
                                     long *Top, long *Left, long *Bottom, long *Right,
                                     long kRx, long kRy, long iR) :
        QCanvasRectangle((int)0, (int)0, (int)*nX, (int)*nY, canvas)
{
    error=false;
    otst=0;
    otstBegX=otstBegY=0;

    nx=nX; ny=nY; ix=iX; iy=iY; left=Left; top=Top; right=Right; bottom=Bottom;
...
/***************************************************************/
{
  ...
  if (kporx && !keyx)
  {
      ...
      prx.sprintf( "*10%+*d", (int)lx, (int)kporx );
      prx = infx + prx;
  }
  else
  {
  ...
  if (kpory && !keyy)
  {
      ...
      pry.sprintf( "*10%+*d", (int)ly, (int)kpory );
      pry = infy + pry;
  }
  else
  {
...
MyCanvasMarker::MyCanvasMarker(QCanvas *canv, double xMin, double xMax, double yMin, double yMax,
                               long iX, long iY, long nX, long nY)
                : QCanvasLine(canv)
{
    canvas=canv;
