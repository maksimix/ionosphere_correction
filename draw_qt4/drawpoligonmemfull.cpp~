...
//
{
  ...
  //cout<<endl<<"1 DrawPoligonMemFull::draw(QPainter &p) listDouble.count()="<<listDouble.count()<<flush;
  ...
  for(; k<nX && listDouble.size(); k++,iList++)
  {
      ...
      //    p.setRasterOp(Qt::OrNotROP);
          //p.setCompositionMode(QPainter::RasterOp_NotSourceOrNotDestination);
