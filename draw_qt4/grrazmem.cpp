...
QSize DrawMemBase::grraz(QPainter &p)
/***************************************************************/
/*                                                             */
...
/***************************************************************/
{
  ...
  */
  ...
  point.ry()=iy;
  point1=point;
  point1.rx()=ix+(nx-otst);
  p.drawLine(point,point1); /* ось абцисс  */
  point1=point;
  point1.ry()=iy-(ny-otst);
  p.drawLine(point,point1); /* ось ординат */
  ...
  if (ipgr==1)
    {
      ...
      point.ry()=iy-(ny-otst);
      point1=point;
      point1.rx()=ix+(nx-otst)-1;
      p.drawLine(point,point1);  /* окантовка графика */
      point=point1;
      point1.rx()=ix+(nx-otst)-1;
      point1.ry()=iy;
      p.drawLine(point,point1);
    }
    ...
    if (mdy==1 || mdy>1 && i%mdy==0)
      {
        ...
        if (ly-ihw>top)
        {
            ...
            //p.drawText( /*iy1-fm.width(rzm+k)-2*/lx, ly+fm.ascent()/2, rzm );
        }
        if (ir==1)
        {
            pen.setColor(colorAxe);
