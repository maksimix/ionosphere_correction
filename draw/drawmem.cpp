...
void DrawMem::initData(long k)
{
    ...
    flagReDraw=true;
    pixGr=0;
    pixGr=new QImage(nx, ny, QImage::Format_ARGB32_Premultiplied);
    keyPoint=0;
    kol=k;
    kolDataX=kolDataY=0;
    x=new double* [kol];
...
void DrawMem::paintEvent(QPaintEvent *e)
{
    ...
    //cout<<endl<<"flagResize="<<flagResize<<endl;
    if (flagReDraw)
    {
        if (pixGr)
        {
        ...
        int dy=fildView.bottom()-fildView.top();
        int dx=fildView.right()-fildView.left();

        pixGr=new QImage(dx, dy, QImage::Format_ARGB32_Premultiplied);
