  ...
  for(i=0; i<n; i++)
  {
      ...
      if (TYPE1)
        for(j=0; j<hop; j++)
        {
            ...
            polar(fr,d_hop1(j),d_hop2(j),ma,psia,
                  fh1,fh2,b1,b2,i1,i2,fox,mc,psic);
            ...
            if (j+1<hop)
              {
                ...
                //      &ref,&ma,&psia);
                rerth(fr,d_hop2(j),mc[ox],psic[ox],epse,sige,
                      &ref,&ma,&psia);
                polr -=ref;
              }
        ...
        else
        {
            ...
            fh1=FH_hop1(hop-1);
            i1=I_hop1(hop-1);
            b1=alf1-D_hop1(hop-1);
            fh2=FH_hop2(hop-1);
            i2=I_hop2(hop-1);
            b2=alf1-D_hop2(hop-1);

            //polar(fr,dhopk[i][hop-1][0],dhopk[i][hop-1][1],ma,psia,
            //      fh1,fh2,b1,b2,i1,i2,fox,mc,psic);
            polar(fr,d_hop1(hop-1),d_hop2(hop-1),ma,psia,
                  fh1,fh2,b1,b2,i1,i2,fox,mc,psic);
            *(mm+i) =mc[ox];
            *(psi+i)=psic[ox];
        }
