...
/*--------------------------------------------------------------*/
   ...
   if (keyOval)
   {
       ...
       rab1=(x-lambd1)/fabs(lambd1);
       rab2=(x+lambd2)/fabs(lambd2);
       rab3=(x+lambd3)/fabs(lambd5);
       rab4=(x+lambd4)/fabs(lambd5);
       double k=1.+amp_proval*(tanh(rab1)-tanh(rab2))+amp_proval1*(tanh(rab4)-tanh(rab3));
       //if (fabs(x)<.5)
       //{
       //    printf("\nro=%f ro_m=%f x=%f lambd1=%f lambd2=%f k=%f",ro,ro_m,x*100.,lambd1,lambd2,k);
       //}
       v*=k;
       double dkx,ch1,ch2,ch3,ch4,dkfi,dkteta;
       ch1=cosh(rab1);
       ch2=cosh(rab2);
       ch3=cosh(rab3);
       ch4=cosh(rab4);
       dkx=amp_proval *(1./(fabs(lambd1)*ch1*ch1)-1./(fabs(lambd2)*ch2*ch2))+
           amp_proval1*(1./(fabs(lambd5)*ch4*ch4)-1./(fabs(lambd5)*ch3*ch3));
