	  ...
	  for(kol=0; kol<5; kol++)
	  {
	      ...
	      if (!(no=soluts(&x,y,n,delt,e,eps,ep,ep1)))
			...
			sg=sqrt(1.-cg*cg);
                        ...
                        a=atan2(y[3],-y[4]);
                        D2(i)=d/RAD;
                        A2(i)=atan2(-y[3],y[4])/RAD; //  azimut prihoda volny
                        //A2(i)=a/RAD; // azimut uhoda volny
                        dhop[i][1]=d;
                        ref[i]=(rot-R0)*100.;
                        FHhop[i][1]=fh;
                        Ihop[i][1]=-asin(h[2]);
                        ...
                        if(i==ihop-1)
                        {
                            if (keytr<0)
                            {
                                ...
                                FILE *outTr;
                                char nameTr[256];
                                sprintf(nameTr,"%05d_%04d_%07d.dat",(int)(fr*1000.),(int)(100.*D1(0)),(int)(100.*A1(0)));
                                outTr=fopen(nameTr,"wt");
                                fprintf(outTr,"%7d\n",pointTr);
                                for(int l=0; l<pointTr; l++)
                                {
                                    fprintf(outTr,"%8.3f  %8.3f\n",90.-tetaTr[l]/RAD,fiTr[l]/RAD);
                                }
                                fclose(outTr);
                            }
                            return no;
