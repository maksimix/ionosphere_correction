...
/*****************************************************************/
{
 ...
 //while(f0[2]>.0 || fabs(y1[2]*100.-6370.-180.)>.1)
 {
      ...
      double ss=.0;
      if ((ss=dist(fiIon0,tetaIon0,y1[0],y1[1]))>RMax)
      {
          static int count=0;
          printf(" ss=%f fiIon0=%f tetaIon0=%f  fi=%f  teta=%f\n",ss,fiIon0/RAD,tetaIon0/RAD,y1[0]/RAD,y1[1]/RAD);
          if (keytr<0)
          {
              count++;
              FILE *outTr;
              char name[256];
              sprintf(name,"trek%04d.dat",count);
              outTr=fopen(name,"wt");
              fprintf(outTr," ss=%f fiIon0=%f tetaIon0=%f  fi=%f  teta=%f\n",ss,fiIon0/RAD,tetaIon0/RAD,y1[0]/RAD,y1[1]/RAD);
              for(int i=0; i<pointTr; i++)
              {
                  fprintf(outTr,"%8.3f  %8.3f  %6.2f  %8.2f\n",fiTr[i]/RAD,tetaTr[i]/RAD,hTr[i],sTr[i]);
              }
              fclose(outTr);
          }
          return -1;
