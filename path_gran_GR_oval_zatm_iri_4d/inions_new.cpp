...
/***************************************************************/
{
  ...
  else
  {
      ...
      iri_3dbase = new IRI_3D(fi0/RAD, 90.-teta0/RAD, rmax, zmin, dz, nx, ny, nz); // основной профиль

      iri_3dbase->setYear(iyear);
      iri_3dbase->setDay(imonth*100+iday);
      iri_3dbase->setTime(time);
      iri_3dbase->setKeyEs((bool)keyes);
      if (keyMod)
          iri_3dbase->setCCIR();
      else
          iri_3dbase->setURSI();
      ...
      iri_3dbase2= new IRI_3D(fi0/RAD, 90.-teta0/RAD, rmax, zmin, dz, nx, ny, nz); // дополнительные профиль

      iri_3dbase2->setYear(iyear);
      iri_3dbase2->setDay(imonth*100+iday);
      iri_3dbase2->setTime(time2);
      iri_3dbase2->setKeyEs((bool)keyes);
      if (keyMod)
          iri_3dbase2->setCCIR();
      else
          iri_3dbase2->setURSI();
