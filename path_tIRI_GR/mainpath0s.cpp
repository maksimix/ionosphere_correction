...
int mainPath(int argc, char *argv[])
{
  ...
  int keyvod, keyMod, keyEs, keypri;
  int i,j,id;
...
/*--------------------------------------------------------------*/
  ...
  fscanf(in,"%d%d%d%d%d%d%d",&keyvod,&keynu,&keyMod,&keyl2,&keyEs,&keypri,&keytr);
  ...
  fscanf(in,"%lg%lg%lg%lg%d%lg%lg%lg%lg%lg",
  ...
  printf("\n1  lon0=%8.3f lat0=%8.3f",lon0/RAD,lat0/RAD);
  ...
  printf("\n2  lon0=%8.3f lat0=%8.3f",lon0/RAD,lat0/RAD);
      ...
      for(id=0; id<kolf; id++)
        {
          ...
          if (iox)
              sprintf(inform," Path: fr=%7.3f  x-ray",fr);
          else
              sprintf(inform," Path: fr=%7.3f  o-ray",fr);
          drawInf(inform);
          ...
          hou=(long)(tim/3600.);
          min=(long)((tim-hou*3600.)/60.);
          sek=(long)(tim-hou*3600.-min*60.);
          msek=(long)((tim-hou*3600.-min*60.-sek)*100.);
