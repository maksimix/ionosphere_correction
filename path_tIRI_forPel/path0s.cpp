...
int main(int argc, char *argv[])
{
    ...
    sprintf(dirnameFreqCity+lenDIRCity,"/%d/Germany/",YEARDIR);

    fileResOut = fopen(fileResName_Copy,"w"); //open results datafile

    lenDIRCity = strlen(dirnameFreqCity);

    sprintf(dirnameFreqCity+lenDIRCity,"%d",freqCity[1]);

    int c = 0;
    filesName = readDir(dirnameFreqCity,&c); // read filenames in folder and write its in array
...
//        sscanf(filesName[k],"%d_%*d_%*d_%*d%*s",&dYear);
    ...
    char* ch1noCor = getInputFile(0,filesName[k],freqCity,hop,City.first, dDay, dMonth,dYear); // files
    char*  ch1 = getInputFile(1,filesName[k],freqCity,hop,City.first, dDay,dMonth,dYear);
