...
//                int TimeCounter = 0;
//                int hop = 3;
//              sscanf(files_name_vector.at(k).c_str(), "%d_%d_%d_%d_%*d%*s", &dYear, &dMonth, &dDay, &dTimeHour);

//              //printf("%d\n",dYear);
//              dTimeHour = (int)ceil(((double)dTimeHour / 100));
//              TimeCounter++;
//              char* ch1noCor = getInputFile(0, files_name_vector.at(k).c_str(), std::stoi(freq), hop, 0, dDay, dMonth, dYear); // files
...
//              if (std::stoi(freq) == 13600 && dTimeHour == 9 && dDay == 7) {
//                printf("%s",files_name_vector.at(k).c_str());
//                printf("time: %d freq:%d\n", dTimeHour, std::stoi(freq));
//                resultMain(ch1noCor, files_name_vector.at(k).c_str(), 0, std::stoi(freq), hop);//Calculate data  without cor
//                free(ch1noCor);
//              }
...
/***************************************************************/
/*                                                             */
/* зПМПЧОБС ЖХОЛГЙС, ПТЗБОЙЪХАЭБС РПУФПТПЕОЙЕ МХЮЕЧПК ФТБЕЛФП- */
/* ТЙЙ Ч УЖЕТЙЮЕУЛЙ УМПЙУФПК ЗПТЙЪПФОБМШОП-ОЕПДОПТПДОПК        */
/* НБЗОЙФПБЛФЙЧОПК ЙПОПУЖЕТЕ.                                  */
/*                                                             */
/*VGG                                                          */
/***************************************************************/
...
/*--------------------------------------------------------------*/
 ...
 sprintf(name, "results/out/%d/%d_%s_", freq, freq,  filePelName.c_str());
 sprintf(nameL, "results/outL/%d/%d_%s", freq, freq, filePelName.c_str());
int strLen = strlen(name);
int strLenL = strlen(nameL);
  ...
  fscanf(in,"%lg%lg%lg%lg%lg",&delt0,&eps,&ep,&ep1,&rmax);
  printf("%lg%lg%lg%lg%lg\n",delt0,eps,ep,ep1,rmax);
  fprintf(out,STR1,delt0,eps,ep,ep1);
  fscanf(in,"%ld%ld%ld%ld%ld%ld%ld",&keyvod,&keynu,&keyMod,&keyl2,&keyEs,&keypri,&keytr);
  printf("%ld%ld%ld%ld%ld%ld%ld\n",keyvod,keynu,keyMod,keyl2,keyEs,keypri,keytr);
  RMax=rmax;
  e=e*eps;
  z0=z1=R0;
  rmax*=.01;
  ...
  fscanf(in,"%lg%lg%lg%lg%ld%lg%lg%lg%lg%lg\n",
         &lon0,&lat0,&z0,&z1,&iyear,&ames,&w,&ig,&ptr,&b);
printf("%lg%lg%lg%lg%ld%lg%lg%lg%lg%lg/n", lon0,lat0,z0,z1,iyear,ames,w,ig,ptr,b);
  fscanf(in,"%lg%lg%lg",&sMinIon,&sMaxIon,&alfIon);
  ...
  if (t1>t2)
    t1-=86400.; // ФЕРЕТШ РПМОПЮШ УПДЕТЦЙФУС ЧОХФТЙ ЙОФЕТЧБМБ НПДЕМЙТПЧБОЙС
  ...
  if(!inPel)
  {
      printf("Error open File:%s",namePel);
      exit(1);
  }
  ...
  while(!feof(inPel))
  {
      ...
      tim=t; // ЧТЕНС ДМС рйч ПФУЮЙФЩЧБЕФУС ПФ 0 (ПФ РПМОПЮЙ ФЕЛХЭЕЗП ДОС)
      //prion(out,lat0,lon0,lat0,lon0,tim,W,IG,keypri,keyEs);
          ...
          // printf("filename: %s\n", file.c_str());
            ...
            for (int k = 0; k < files_name_vector.size(); k++) {
              ...
              //free(ch1);
                ...
                printf("%s",ch1noCor);
                printf("time: %d freq:%d\n", dTimeHour, std::stoi(freq));
                resultMain(ch1noCor, files_name_vector.at(k).c_str(), 0, std::stoi(freq));//Calculate data  without cor
               char* argv[] = { (char*)"./path0s", ch1noCor, (char*)"out636.txt" };
