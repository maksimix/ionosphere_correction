...
DrawCanvas::DrawCanvas(QCanvas *canvas, QWidget *parent,
                       long k,
                       //char *infX, char *infY,
                       ...
                       DrawBaseCanvas(canvas, parent, infX, infY, xMin, xMax, yMin, yMax,
                               iR, iPgr, kRx, kRy)
{
    //cout<<endl<<"222222222222222222222222222"<<flush;
    initData(k);
}
...
void DrawCanvas::initData(long k)
{
  ...
  for(i=0; i<KolMetka; i++)
  {
      ...
      lineMetka[i]=new QCanvasLine(canvas);
...
bool DrawCanvas::resetData(bool flag)
{
    if (!flagMinMax)
    {
...
//
{
  ...
  int step=size.height()/(KolMetka+1);
  for(i=0; i<KolMetka; i++)
  {
      y1=y0=iy+step/2+step*i;
      x0=(long)((x1Metka[i]-xmin)*(nx-otst)/sizex)+ix;
      x1=(long)((x2Metka[i]-xmin)*(nx-otst)/sizex)+ix;
  ...
  for(i=0; i<kol; i++)
  {
      ...
      QPointArray &array=diagramma[i]->getPointArray();
      array.resize(n[i]);
      nPoint=0;
      x1=y1=0;
      flag=true;
      ...
      {
          if (x[i])
              x1=(long)((x[i][j]-xmin)*(nx-otst)/sizex)+ix;
          else
              x1=(long)((xBeg+Dx*j-xmin)*(nx-otst)/sizex)+ix;
          if (y[i])
              y1=-(long)((y[i][j]-ymin)*(ny-otst)/sizey)+iy-otst;
          else
              y1=-(long)((yBeg+Dy*j-ymin)*(ny-otst)/sizey)+iy-otst;
          if (x1>=ix && x1<=ix+nx && y1<=iy-1 && y1>=iy-ny-1)
          {
              if (!nPoint)
              {
                  xBegin[i]=x0=x1;
...
void MyCanvasPolyline::drawShape( QPainter & painter )
{
    ...
    if (keyPoint>0)
    {
        //cout<<endl<<"!!!!!!!!!!!!!!!!!! keyPoint="<<keyPoint<<flush;
