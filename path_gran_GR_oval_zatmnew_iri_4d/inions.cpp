...
double tZat=.0;
double tZatBeg0,tZatBeg1,tZatEnd1,tZatEnd0;
...
/***************************************************************/
{
  ...
  if (keyZat)
  {
      fscanf(in,"%lg%lg%lg%lg%lg",&tZat,&tZatBeg0,&tZatBeg1,&tZatEnd1,&tZatEnd0);
      tZatBeg0*=3600.;
      tZatBeg1*=3600.;
      tZatEnd1*=3600.;
      tZatEnd0*=3600.;
      ...
      FILE *outZ;
      int len=strlen(name);
      sprintf(name+len-4,"_0.dat");
      outZ=fopen(name,"wt");;
      fprintf(outZ,"%5d\n",keyZat);
      for(int i=0; i<keyZat; i++)
          fprintf(outZ,"%8.3f  %8.3f\n",latCzat[i],lonCzat[i]);
      fclose(outZ);
      sprintf(name+len-4,"_1.dat");
      outZ=fopen(name,"wt");;
      fprintf(outZ,"%5d\n",keyZat);
      for(int i=0; i<keyZat; i++)
          fprintf(outZ,"%8.3f  %8.3f\n",latCzat1[i],lonCzat1[i]);
      fclose(outZ);
      sprintf(name+len-4,"_2.dat");
      outZ=fopen(name,"wt");;
      fprintf(outZ,"%5d\n",keyZat);
      for(int i=0; i<keyZat; i++)
      ...
      double lat_0;
      fscanf(in,"%lg%lg%lg%lg%lg%lg%lg",&delta0,&delta1,&r_0,&fi_0,&lat_0,&aX,&aZ);
      fprintf(out,"\ndelta0=%5.3f  delta1=%5.3f  r0=%6.1f  lon=%8.3f lat=%8.3f",delta0,delta1,r_0,fi_0,lat_0);
      fprintf(out,"\naX=%6.1f  aZ=%6.1f\n",aX,aZ);
      keyFsp=0; // tolko zatmenie
      r_0=R0+r_0*.01;
      teta_0=RAD*(90.-lat_0);
      fi_0*=RAD;
      aX*=.01;
  ...
  else
  {
      if (keyFsp>0)
      {
          ...
          fscanf(in,"%lg%lg%lg%lg%lg%lg%lg%lg%lg%lg",&delta0,&delta1,&r_0,&fi_0,&lat_0,&beta0,&alfa0,&aX,&aY,&aZ);
          fprintf(out,"\ndelta0=%5.3f  delta1=%5.3f  r0=%6.1f  lon=%8.3f lat=%8.3f",delta0,delta1,r_0,fi_0,lat_0);
          ...
          sBeta0=sin(beta0);
          cBeta0=cos(beta0);
          A[0][0]=-sin(fi_0); A[0][1]=-cos(teta_0)*cos(fi_0); A[0][2]=sin(teta_0)*cos(fi_0);
          A[1][0]= cos(fi_0); A[1][1]=-cos(teta_0)*sin(fi_0); A[1][2]=sin(teta_0)*sin(fi_0);
  ...
  if (keyOval>0)
  {
      ...
      lambd1=20.;
      lambd1=5.;
      fscanf(in,"%lg%lg%lg%lg",&fi0_inv,&amp_proval,&lambd1,&lambd2);
