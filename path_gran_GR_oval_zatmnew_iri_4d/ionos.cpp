...
/*--------------------------------------------------------------*/
   ...
   if (r<h0)
     ...
     if (keyFsp)
     {
         ...
         double dlt,dDlt,dDlt1,sF_A,cF_A;
         ...
         double rabr,rabf,rabt,rabr1,rabf1,rabt1;
         ...
         rabr1=-2.*(x1*dx1r+y1*dy1r);
         rabf1=-2.*(x1*dx1f+y1*dy1f);
         rabt1=-2.*(x1*dx1t+y1*dy1t);
         rabr=-2.*(x1*dx1r+y1*dy1r+z1*dz1r);
         rabf=-2.*(x1*dx1f+y1*dy1f+z1*dz1f);
         rabt=-2.*(x1*dx1t+y1*dy1t+z1*dz1t);

         dlt_r=dDlt*rabr+dDlt1*rabr1;
         dlt_f=dDlt*rabf+dDlt1*rabf1;
         dlt_t=dDlt*rabt+dDlt1*rabt1;

         dlt_rr=dDlt*(rabr*rabr-2.*((x1*dx1rr+dx1r*dx1r)+(y1*dy1rr+dy1r*dy1r)+(z1*dz1rr+dz1r*dz1r)))+
                dDlt1*(rabr1*rabr1-2.*((x1*dx1rr+dx1r*dx1r)+(y1*dy1rr+dy1r*dy1r)));
         dlt_ff=dDlt*(rabf*rabf-2.*((x1*dx1ff+dx1f*dx1f)+(y1*dy1ff+dy1f*dy1f)+(z1*dz1ff+dz1f*dz1f)))+
                dDlt1*(rabf1*rabf1-2.*((x1*dx1ff+dx1f*dx1f)+(y1*dy1ff+dy1f*dy1f)));
         dlt_tt=dDlt*(rabt*rabt-2.*((x1*dx1tt+dx1t*dx1t)+(y1*dy1tt+dy1t*dy1t)+(z1*dz1tt+dz1t*dz1t)))+
                dDlt1*(rabt1*rabt1-2.*((x1*dx1tt+dx1t*dx1t)+(y1*dy1tt+dy1t*dy1t)));
         dlt_rf=dDlt*(rabr*rabf-2.*((x1*dx1rf+dx1r*dx1f)+(y1*dy1rf+dy1r*dy1f)+(z1*dz1rf+dz1r*dz1f)))+
                dDlt1*(rabr1*rabf1-2.*((x1*dx1rf+dx1r*dx1f)+(y1*dy1rf+dy1r*dy1f)));
         dlt_rt=dDlt*(rabr*rabt-2.*((x1*dx1rt+dx1r*dx1t)+(y1*dy1rt+dy1r*dy1t)+(z1*dz1rt+dz1r*dz1t)))+
                dDlt1*(rabr1*rabt1-2.*((x1*dx1rt+dx1r*dx1t)+(y1*dy1rt+dy1r*dy1t)));
         dlt_ft=dDlt*(rabf*rabt-2.*((x1*dx1ft+dx1f*dx1t)+(y1*dy1ft+dy1f*dy1t)+(z1*dz1ft+dz1f*dz1t)))+
                dDlt1*(rabf1*rabt1-2.*((x1*dx1ft+dx1f*dx1t)+(y1*dy1ft+dy1f*dy1t)));
     ...
     {
         double eps=1.e-16;
         double sinDfi,cosDfi,Dfi;
         double cost0,sint0,sinff0,cosff0;
         double RaX2,RaX,aZ2;
         double x1,z1;
         double dlt,dDlt,dDlt1;
         double dDfi_f,dDfi_t,ddDfi_ff,ddDfi_tt,ddDfi_ft;
         double rab_r,rab_f;
         double dlt_r,dlt_f,dlt_t,dlt_rr,dlt_tt,dlt_ff,dlt_rf,dlt_rt,dlt_ft;

         sint0=sin(teta_0);
         cost0=cos(teta_0);
         sinff0=sin(f-fi_0);
         cosff0=cos(f-fi_0);
         cosDfi=ct*cost0+st*sint0*cosff0;
         sinDfi=sqrt(1.-cosDfi*cosDfi);
         Dfi=atan2(sinDfi,cosDfi);
         RaX=R0/aX; RaX2=RaX*RaX;
         aZ2=aZ*aZ;
         x1=RaX*Dfi;
         z1=(r-r_0)/aZ;
         dDlt1=delta1*exp(-x1*x1);
         dDlt =delta0*exp(-x1*x1-z1*z1);
         dlt=1.+dDlt+dDlt1;
         dDfi_f=sint0*st*sinff0/sinDfi;
         dDfi_t=(cost0*st-sint0*ct*cosff0)/sinDfi;
         ddDfi_ff=(sint0*st*cosff0-cosDfi*dDfi_f*dDfi_f)/sinDfi;
         ddDfi_tt=(cost0*ct+sint0*st*cosff0-cosDfi*dDfi_t*dDfi_t)/sinDfi;
         ddDfi_ft=(sint0*st*sinff0-cosDfi*dDfi_f*dDfi_t)/sinDfi;
         rab_r=-2.*z1/aZ;
         rab_f=-2.*x1*RaX;

         dlt_r = rab_r*dDlt;
         dlt_f =(dDlt+dDlt1)*rab_f*dDfi_f;
         dlt_t =(dDlt+dDlt1)*rab_f*dDfi_t;
         dlt_rr=dDlt*(rab_r*rab_r-2./aZ2);
         dlt_ff=(dDlt+dDlt1)*((rab_f*rab_f-2.*RaX2)*dDfi_f*dDfi_f+rab_f*ddDfi_ff);
         dlt_tt=(dDlt+dDlt1)*((rab_f*rab_f-2.*RaX2)*dDfi_t*dDfi_t+rab_f*ddDfi_tt);
         dlt_rf=dDlt*rab_f*rab_r*dDfi_f;
         dlt_rt=dDlt*rab_f*rab_r*dDfi_f;
         dlt_ft=(dDlt+dDlt1)*((rab_f*rab_f-2.*RaX2)*dDfi_f*dDfi_t+rab_f*ddDfi_ft);
