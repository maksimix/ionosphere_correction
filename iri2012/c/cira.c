...
struct parm7_1_ {
    real pt[150], pda1[150], pda2[150], pda3[150], pda4[150], pda5[150], pda6[
	    150], pda7[150], pda8[150], pda9[150], ps[150], pdl[50]	/* 
...
/* Neutral temperature and density model used in IRI */

/* Includes the following subroutines and functions: */
/*    GTD7, GTD7D, GHP7, GLATF, VTST7, GTS7, METERS, SCALH, GLOBE7, TSELEC, */
/*    GLOBE7S, DENSU, DENSM, SPLINEM, SPLINTM, SPLINI,DNET, CCOR, CCOR2, */
/*    BLOCKDATA GTD7BK */
...
/* Subroutine */ int gtd7_(iyd, sec, alt, glat, glong, stl, f107a, f107, ap, 
	mass, d__, t)
integer *iyd;
...
/*      Put identification data into common/datime/ */
    for (i__ = 1; i__ <= 3; ++i__) {
	s_copy(datime_1.isdate + (i__ - 1 << 2), datim7_1.isd + (i__ - 1 << 2)
		, (ftnlen)4, (ftnlen)4);
/* L1: */
    }
    for (i__ = 1; i__ <= 2; ++i__) {
	s_copy(datime_1.istime + (i__ - 1 << 2), datim7_1.ist + (i__ - 1 << 2)
		, (ftnlen)4, (ftnlen)4);
	s_copy(datime_1.name__ + (i__ - 1 << 2), datim7_1.nam + (i__ - 1 << 2)
		, (ftnlen)4, (ftnlen)4);
/* L2: */
    }
...
/* Subroutine */ int gtd7d_(iyd, sec, alt, glat, glong, stl, f107a, f107, ap, 
	mass, d__, t)
integer *iyd;
real *sec, *alt, *glat, *glong, *stl, *f107a, *f107, *ap;
integer *mass;
...
/* Subroutine */ int ghp7_(iyd, sec, alt, glat, glong, stl, f107a, f107, ap, 
	d__, t, press)
integer *iyd;
...
doublereal vtst7_(iyd, sec, glat, glong, stl, f107a, f107, ap, ic)
integer *iyd;
...
/* Subroutine */ int gts7_(iyd, sec, alt, glat, glong, stl, f107a, f107, ap, 
	mass, d__, t)
integer *iyd;
real *sec, *alt, *glat, *glong, *stl, *f107a, *f107, *ap;
integer *mass;
real *d__, *t;
{
    ...
    /* Local variables */
    static real hcc32, hcc14, hcc16, db16h, zcc01, zcc32, zcc14;
    ...
    static real zhm01, tinf, zhm04, zhm32, zhm16, zhm40, zhm14, zhm28, zmho, 
	    zsho, hcc232, zsht;
    extern doublereal ccor2_();
    static integer i__, j;
    extern doublereal vtst7_();
    static real z__;
    extern doublereal scalh_(), densu_();
    static real tnmod, g1, g4, t2, v2;
    extern doublereal globe7_(), glob7s_();
    static real b01, b04, b32, b16, g40, b28, g32, g16, b40, g14, g28, b14, 
	    tz, hc04, hc32, hc16, rc32, rc16, day, zh04, zhf, zh28, yrd, xmm, 
	    xmd, zc04, zh16, zc16, zh32, zc32, zh40, hc40, zc40, zh01, hc01, 
	    zc01, rc01, hc216, zh14, hc14, zc14, rc14, g16h, tho, hcc01;

    /* Fortran I/O blocks */
    static cilist io___74 = { 0, 6, 0, fmt_100, 0 };
...
/*      Calculate these temperatures only if input changed */
	...
	gts3c_1.tlb = lower7_1.ptm[1] * (csw_1.sw[16] * globe7_(&yrd, sec, 
		glat, glong, stl, f107a, f107, &ap[1], parm7_1.pda4) + (float)
		1.) * parm7_1.pda4[0];
    }
...
/*      Diffusive density at Zlb */
    gts3c_1.db28 = lower7_1.pdm[20] * exp(g28) * parm7_1.pda3[0];
...
/*       Density variation factor at Zlb */
    g4 = csw_1.sw[20] * globe7_(&yrd, sec, glat, glong, stl, f107a, f107, &ap[
	    1], parm7_1.pda1);
/*      Diffusive density at Zlb */
    gts3c_1.db04 = lower7_1.pdm[0] * exp(g4) * parm7_1.pda1[0];
/*      Diffusive density at Alt */
...
/*      DB16 =  PDM(1,2)*EXP(G16)*PDA2(1) */
    gts3c_1.db16 = lower7_1.pdm[10] * exp(g16) * parm7_1.pda2[0];
/*       Diffusive density at Alt */
...
/*       Density variation factor at Zlb */
    g32 = csw_1.sw[20] * globe7_(&yrd, sec, glat, glong, stl, f107a, f107, &
	    ap[1], parm7_1.pda5);
/*      Diffusive density at Zlb */
    gts3c_1.db32 = lower7_1.pdm[30] * exp(g32) * parm7_1.pda5[0];
/*       Diffusive density at Alt */
...
/*       Density variation factor at Zlb */
    g40 = csw_1.sw[20] * globe7_(&yrd, sec, glat, glong, stl, f107a, f107, &
	    ap[1], parm7_1.pda6);
/*      Diffusive density at Zlb */
    gts3c_1.db40 = lower7_1.pdm[40] * exp(g40) * parm7_1.pda6[0];
/*       Diffusive density at Alt */
...
/*       Density variation factor at Zlb */
    g1 = csw_1.sw[20] * globe7_(&yrd, sec, glat, glong, stl, f107a, f107, &ap[
	    1], parm7_1.pda7);
/*      Diffusive density at Zlb */
    gts3c_1.db01 = lower7_1.pdm[50] * exp(g1) * parm7_1.pda7[0];
/*       Diffusive density at Alt */
...
/*       Density variation factor at Zlb */
    g14 = csw_1.sw[20] * globe7_(&yrd, sec, glat, glong, stl, f107a, f107, &
	    ap[1], parm7_1.pda8);
/*      Diffusive density at Zlb */
    gts3c_1.db14 = lower7_1.pdm[60] * exp(g14) * parm7_1.pda8[0];
/*       Diffusive density at Alt */
...
/*        **** Anomalous OXYGEN DENSITY **** */
    ...
    g16h = csw_1.sw[20] * globe7_(&yrd, sec, glat, glong, stl, f107a, f107, &
	    ap[1], parm7_1.pda9);
    db16h = lower7_1.pdm[70] * exp(g16h) * parm7_1.pda9[0];
...
doublereal globe7_(yrd, sec, lat, long__, tloc, f107a, f107, ap, p)
real *yrd, *sec, *lat, *long__, *tloc, *f107a, *f107, *ap, *p;
{
    ...
    extern /* Subroutine */ int tselec_();
    static real cd14, cd32, cd18, cd39, exp1;
...
.1)";
    ...
    /* Fortran I/O blocks */
    static cilist io___185 = { 0, 6, 0, fmt_900, 0 };
...
real *zn1, *tn1, *tgn1;
{
    ...
    extern /* Subroutine */ int splini_();
    static real yd1, yd2, zg2, glb, dta;
    extern /* Subroutine */ int splinem_(), splintm_();
...
/*       Calculate spline coefficients */
    splinem_(xs, ys, &mn, &yd1, &yd2, y2out);
    x = zg / zgdif;
    splintm_(xs, ys, y2out, &mn, &x, &y);
/*       temperature at altitude */
    *tz = (float)1. / y;
    ret_val = *tz;
L10:
...
/* Subroutine */ int splinem_(x, y, n, yp1, ypn, y2)
real *x, *y;
integer *n;
real *yp1, *ypn, *y2;
{
...
/* Subroutine */ int splintm_(xa, ya, y2a, n, x, y)
real *xa, *ya, *y2a;
integer *n;
real *x, *y;
{
...
doublereal ccor2_(alt, r__, h1, zh, h2)
real *alt, *r__, *h1, *zh, *h2;
{
    /* System generated locals */
    real ret_val;
