...
/*--------------------------------------------------------------*/
  ...
  it1=000000l; // начальный момент интервала поиска
  it2=240000l; // конечный момент интервала поиска
  it3=003000l; // шаг по времени для поиска моментов восхода-захода

  fscanf(in,"%lg",&f); // ввод частоты
  putdat=(PUTDAT**)malloc(sizeof(PUTDAT*)*2); // распределение памяти для результатов расчета
                                              // характеристик распространения в окрестности МПЧ
  putdat[0]=(PUTDAT*)malloc(sizeof(PUTDAT)*hop2*maxKor);
  putdat[1]=(PUTDAT*)malloc(sizeof(PUTDAT)*hop2*maxKor);
  double tVoshod=.0, tZahod=.0;  // моменты восхода и захода
  ...
  if (flag)
  {
      ...
      fprintf(out,"\nМомент восхода (ччммсс)=%d,  в секундах %f",t,tVoshod);

      hou=tZahod/3600.;
      min=(tZahod-hou*3600.)/60.;
      sek=tZahod-hou*3600.-min*60.;
      t=hou*10000l+min*100l+sek+.5;
      fprintf(out,"\nМомент захода (ччммсс)=%d,  в секундах %f",t,tZahod);
  ...
  {
    fprintf(out,"Для частоты %6.3f момент восхода и захода не определены",f);
  }
