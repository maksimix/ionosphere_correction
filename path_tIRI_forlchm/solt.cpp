...
#define FORM1 "\nd8,d7,s,fr,hop,no %12.8f%7.2f%13.6f%6.2f%3d%3d"
#define FORM2 "\nd6,d7,s,koli,no %12.8lf%7.2lf%13.6lf%13.6lf%3d%3d"
...
// дальность по Земле не станет больше длины трассы
  ...
  //printf("\n0  d0=%f alfa=%f dd=%f s=%f no=%d io=%d",d8/RAD,alfa/RAD,dd/RAD,s*100.,no,io);
  ...
  if (no==-1)
  {
      ...
      if (!io)
         dd=-dd;
  //printf("\n1  d0=%f alfa=%f dd=%f no=%d io=%d",d8/RAD,alfa/RAD,dd/RAD,no,io);

  while (fabs(dd)>EPS)
  {
      //printf("\n2  d0=%f alfa=%f dd=%f no=%d io=%d",d8/RAD,alfa/RAD,dd/RAD,no,io);
      if (s<eps2  || d8>d1)
      {
          return n;
      }
          ...
          if ((d6=zero_t(d2,f2,d3,f3,s0,alfa,ihop,delt,eps,ep,ep1,e,eps1,epsa,aa0,putdat+ihop*n))>=.0)
            {
              if (keysol<0) keytr=-1;
              //keytr=1;
              //no=treks(d6,alfa,ihop,22,delt,eps,ep,ep1,e,putdat+ihop*n);
              //no=treks(d6,(AT(ihop*n+ihop-1)-DEV(ihop*n+ihop-1))*RAD,ihop,22,delt,eps,ep,ep1,e,putdat+ihop*n);
              no=treks(d6,A1(ihop*n+ihop-1)*RAD,ihop,22,delt,eps,ep,ep1,e,putdat+ihop*n);
              //printf("\n2  d6=%6.3f d7=%6.3f s=%8.3f dd=%6.3f io=%2d n0=%2d",d6/RAD,D2(ihop*n+ihop-1),S(ihop*n+ihop-1),dd/RAD,io,no);
              //keytr=0;
              if (keysol<0) keytr=0;
...
*/
      ...
      { /* no */
          do
          {
              ...
              s=S(ihop*n+ihop-1);
              d7=D2(ihop*n+ihop-1)*RAD;
              //printf("\n1  d8=%6.3f d7=%6.3f s=%8.3f dd=%6.3f io=%2d n0=%2d",d8/RAD,d7/RAD,s,dd/RAD,io,no);
    ...
    { /* wh */
      ...
      { /* do */
                ...
                && koli<=MAXIT)
          {
              ...
              if (keysol>0)
                printf(FORM2,b/RAD,D2(ihop-1),S(ihop-1),s0*100.,koli,no);
          }
          ...
          if (keysol>0)
            printf(FORM2,b/RAD,D2(ihop-1),s1,s0*100.,koli,no);
