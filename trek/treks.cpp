                   ...
                   PUTDAT *putdat)
{
    ...
    u1=fh/fr;
    u=u1*u1;
    return treks(d0,alfa,ihop,n,putdat);
}
...
{
    tim=t;
    return treks(f,d0,alfa,ihop,n,putdat);
}

int  MyTrek::treks(double d0,
                   double alfa,
                   int    ihop,
                   int    n,
                   PUTDAT *putdat)
/*******************************************************************/
/*                                                                 */
/*     ПОДПРОГРАММА ПОДГОТАВЛИВАЕТ НАЧАЛЬНЫЕ ДАННЫЕ ДЛЯ РЕШЕНИЯ    */
/*     УРАВНЕНИЙ ЛУЧА И ВЫЧИСЛЯЕТ ИНТЕГРАЛЬНЫЕ ХАРАКТЕРИСТИКИ      */
/*     МНОГОСКАЧКОВОЙ ТРАЕКТОРИИ.                                  */
/*                                                                 */
/*  В ЭТОЙ ПОДПРОГРАММЕ В КАЧЕСТВЕ ПАРАМЕТРОВ ДЛЯ ПРИСОЕДИНЕННОЙ   */
/*       СИСТЕМЫ УРАВНЕНИЙ ВЫБРАНЫ УГЛЫ ИЗЛУЧЕНИЯ                  */
/*                                                                 */
/* d0   -УГОЛ ИЗЛУЧЕНИЯ;                                           */
/* alfa -АЗИМУТ ИЗЛУЧЕНИЯ;                                         */
/* ihop -ЧИСЛО СКАЧКОВ;                                            */
/* n    -КОЛИЧЕСТВО ИНТЕГРИРУЕМЫХ УРАВНЕНИЙ;                       */
/* delt0,eps,ep,ep1,e-ТОЧНОСТНЫЕ ХАРАКТЕРИСТИКИ;                   */
/* *putdat-МАССИВ ВЫХОДНЫХ ИНТЕГРАЛЬНЫХ ХАРАКТЕРИСТИК.             */
/*                                                                 */
/*VGG                                                              */
/*******************************************************************/
{
  ...
  for(i=0; i<ihop; i++)
  {
      ...
      Ref(i)=d_hop1(i)=d_hop2(i)=.0;
      D_hop1(i)=D_hop2(i)=I_hop1(i)=I_hop2(i)=FH_hop1(i)=FH_hop2(i)=.0;
      Moda(i)=0;
      if (i<ihop-1)
      {
          rE=R0;
          ...
          for(kol=0; kol<5; kol++)
          {
                ...
                if (!(no=soluts(&x,y,n,delt,e,eps,ep,ep1)))
                {
                        ...
                        if (!one)
                        {
                            ...
                            //printf("\nL(i)=%6.2f  i=%2d  DF(i)=%7.4f PG(i)=%7.1f  PS(i)=%7.1f  n=%2d",L(i),i,DF(i),PG(i),PS(i),n);
...
double MyTrek::dist(double fi0, double teta0, double fi1, double teta1)
{
